...
def _(np, softmax):
    ...
    Q = _rng.standard_normal((seq_len, d_k))  # queries
    K = _rng.standard_normal((seq_len, d_k))  # keys
    V = _rng.standard_normal((seq_len, d_k))  # values
...
def _(np):
    def _run():
        ...
        for i in range(len(masked)):
            if _rng.random() < mask_prob or i == 2:  # force at least one mask
                masked[i] = "[MASK]"
...
def _(np):
    def _run():
        ...
        query_emb = doc_embeddings[2] + _rng.standard_normal(16) * 0.3
...
def _(np):
    ...
    # _rng = np.random.default_rng(42)
    # seq, dm = 6, 16
    # Q_test = _rng.standard_normal((seq, dm))
    # K_test = _rng.standard_normal((seq, dm))
    # V_test = _rng.standard_normal((seq, dm))
